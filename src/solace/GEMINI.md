Create a Solace health checker, using as template the Postgres checker:
1. Create a folder src/solace
2. Create a go library there (with go.mod and friends)
3. Add the library to the @go.work file.
4. Make sure that the library depends on the official Solace client library for Go.
...
6. Create integration tests with 100% test coverage (instead of just using unit tests) by using Testcontainers (either the vanilla one
   or the one for Solace, if such exists). 
7. Add fuzz tests as well.
8. Add load tests, which verify that the Health() routine can handle at least 200 calls per second using a real Solace connection (instead of
   mocking it).
