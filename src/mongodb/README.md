...
This Go module provides a health checker for MongoDB databases, designed to be integrated into a larger health-check system. It continuously monitors the availability and responsiveness of a MongoDB instance by periodically executing a `Ping` command.
...
*   **Testcontainers Integration:** Integration tests leverage Testcontainers for spinning up real MongoDB instances, ensuring robust and realistic testing.
...
Here's a basic example of how to initialize and use the MongoDB health checker:
...
func main() {
	// Define a descriptor for your MongoDB component
	...
	defer func() {
		if err := checker.Close(); err != nil {
			log.Printf("Error closing MongoDB checker: %v", err)
		}
	}()
	...
	go func() {
		for status := range checker.StatusChange() {
			fmt.Printf("MongoDB Status Changed: %s - %s (Latency: %.2f%s)\n",
				status.Status,
				status.Output,
				status.ObservedValue,
				status.ObservedUnit)
		}
	}()
	...
	for i := 0; i < 10; i++ {
		...
		fmt.Printf("Current MongoDB Health: %s - %s\n", health.Status, health.Output)
...
**Note:** Integration tests require Docker to be running, as they spin up a real MongoDB container using Testcontainers.
...
```bash
go test -fuzz=FuzzMongoChecker -fuzztime=30s ./...
```
...
```bash
go test -run TestMongoChecker_HealthLoad_WithRealDB -count=1 -v -test.parallel 1 ./...
```

This will run the load test against a real MongoDB instance spawned by Testcontainers, simulating concurrent calls to the `Health()` method.

## Configuration

The `NewMongoChecker` function takes the following parameters:
...
*   `connectionString string`: The connection URI for connecting to the MongoDB database.
...
Performance benchmarks from load tests (e.g., `TestMongoChecker_HealthLoad_WithRealDB`) aim to verify that the `Health()` routine can handle at least 200 calls per second, even under adverse conditions such as the database temporarily going down. The actual health check (`Ping` command) runs in a separate goroutine on a defined interval, ensuring the `Health()` method itself remains performant.
