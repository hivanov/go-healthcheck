...
func TestPostgresChecker_Integration_Fail_NoConnection(t *testing.T) {
	...
	checker := NewPostgresChecker(desc, checkInterval, 1*time.Second, invalidConnStr)
	defer func() {
		assert.NoError(t, checker.Close(), "Checker Close() returned an unexpected error")
	}()
...
func TestPostgresChecker_Integration_Fail_DBDown(t *testing.T) {
	...
	require.NoError(t, pgContainer.Stop(ctx, nil), "Failed to stop PostgreSQL container")
...
func TestPostgresChecker_Integration_DisableEnable(t *testing.T) {
	...
	require.Equal(t, "PostgreSQL checker disabled", status.Output)
	...
	require.Contains(t, statusAfterSleep.Output, "PostgreSQL checker disabled", "Expected output to remain 'PostgreSQL checker disabled'")

	// Enable the checker
	checker.Enable()
	status = checker.Status()
	require.Equal(t, core.StatusWarn, status.Status)
	require.Equal(t, "PostgreSQL checker enabled, re-initializing...", status.Output)
...
func TestPostgresChecker_Integration_Close(t *testing.T) {
	...
	require.Eventually(t, func() bool {
		select {
		case <-checker.(*postgresChecker).ctx.Done():
			return true
		default:
			return false
		}
...
func TestNewPostgresChecker_OpenError(t *testing.T) {
	...
	checker := NewPostgresCheckerWithOpenDBFunc(desc, checkInterval, 1*time.Second, connectionString, mockOpenDB)
	defer func() {
		assert.NoError(t, checker.Close(), "Checker Close() returned an unexpected error")
	}()
...
func TestPostgresChecker_PerformHealthCheck_ResultNotOne(t *testing.T) {
	...
	require.Contains(t, status.Output, "PostgreSQL health check query returned unexpected result: 0 (expected 1)", "Expected specific output")
}
...
func TestPostgresChecker_PerformHealthCheck_ScanError(t *testing.T) {
	...
	require.Contains(t, status.Output, "PostgreSQL health check query failed", "Expected specific output")
